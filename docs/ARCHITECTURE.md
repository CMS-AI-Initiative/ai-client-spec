...
Below you find the full class diagram. Since it covers a lot at once, further below you also find "zoomed-in" variants of specific subsystems. Most importantly, the APIs for _using_ generative AI and the APIs for _registering_ (and implementing) generative AI providers are intentionally separated, so that changes in one do not affect the other. Additionally, since the majority of developers will be _using_ generative AI (rather than implementing providers), the primary focus on API developer experience is on generative AI _usage_.
...
---
%% DO NOT EDIT THIS!
%% Instead, edit the FULL class diagram above and copy the relevant updates over here.
classDiagram
direction LR
    namespace Ai {
        class AiEntrypoint {
